...
chatbot_service.py
-------------------
The "brain" of the FitMacros Fitness Coach chatbot. It answers questions
from three sources, tried in order:
  ...
  2. General fitness education - pulled from the JSON knowledge base and
     matched using keyword_matcher.py. This is free, instant, and covers
     the vast majority of fitness questions.
  3. OPTIONAL AI fallback (ai_fallback.py) - only reached if step 2 finds
     absolutely no match. Only does anything if GEMINI_API_KEY is set; if
     it isn't, this step is skipped entirely and the original static
     fallback message is used instead, exactly as before.

Steps 1 and 2 never call any external API - they're plain Python logic
and a JSON knowledge base. Step 3 is the only place an external AI
service is ever used, and it's fully optional.
"""
...
def handle_message(message: str, context: dict = None) -> dict:
    ...
    # 4. Nothing matched the free knowledge base. Try the optional AI
    # fallback (only does anything if GEMINI_API_KEY is configured);
    # otherwise fall back to the original static message, unchanged.
    ai_reply = ai_fallback.get_ai_reply(message, context)
    if ai_reply:
        return {"reply": ai_reply, "topic": "AI Assistant"}
