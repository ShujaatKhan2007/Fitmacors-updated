...
import PostureChecker from "./PostureChecker.jsx";

const REST_DURATION_OPTIONS = [30, 45, 60, 90, 120];

/**
 * Turns a rep range like "10-12" into a single target number the posture
 * checker can count towards. Uses the LOWER end of the range, so the set
 * completes once they've hit the minimum with good form.
 */
...
export default function WorkoutPlayer({ day, onExit }) {
  ...
  const [isCheckingPosture, setIsCheckingPosture] = useState(false);
  ...
  return (
    <div className="coach-overlay" role="dialog" aria-label="Workout Coach Mode">
      <div className="coach-overlay__inner">
        ...
        {phase === "exercising" && currentExercise && (
          <>
            ...
            <ExerciseCard
              ...
              isPaused={isPaused || isCheckingPosture}
              onCompleteSet={handleCompleteSet}
              onCheckPosture={() => setIsCheckingPosture(true)}
            />
            <WorkoutControls
        ...
        {isCheckingPosture && currentExercise && (
          <PostureChecker
            exercise={currentExercise}
            targetReps={parseTargetReps(currentExercise.reps)}
            durationSeconds={currentExercise.duration_seconds}
            onClose={() => setIsCheckingPosture(false)}
            onComplete={() => {
              setIsCheckingPosture(false);
              handleCompleteSet();
            }}
          />
        )}
