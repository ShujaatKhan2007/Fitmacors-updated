...
# ---------------------------------------------------------------------------
# Food Photo Analysis request/response models.
# ---------------------------------------------------------------------------

class FoodImageRequest(BaseModel):
    # Raw base64 image data - no "data:image/jpeg;base64," prefix, just
    # the encoded bytes themselves. The frontend strips that prefix
    # before sending.
    image_base64: str = Field(..., description="Base64-encoded image data (no data: URL prefix)")
    mime_type: str = Field("image/jpeg", description="Image MIME type, e.g. image/jpeg or image/png")
