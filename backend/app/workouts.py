...
from app import exercise_guidance
...
def _build_exercise(
    ...
    reps: str,
    rest_seconds: int,
    fitness_level_value: str,
) -> Exercise:
    """
    Builds one fully enriched Exercise, adding everything Workout Coach
    Mode needs: instructions, common mistakes, animation filename, and -
    for exercises like Plank that are held rather than counted in reps -
    a duration instead of a rep range.
    """
    guidance = exercise_guidance.get_guidance(name)
    time_based = exercise_guidance.is_time_based(name)

    return Exercise(
        name=name,
        sets=sets,
        # Time-based exercises (Plank, Wall Sit, ...) don't have a rep
        # range - the countdown timer in Coach Mode shows the duration
        # instead, so we leave this blank for those.
        reps="" if time_based else reps,
        rest_seconds=rest_seconds,
        muscle_group=muscle_group,
        is_time_based=time_based,
        duration_seconds=(
            exercise_guidance.get_duration_seconds(name, fitness_level_value)
            if time_based
            else None
        ),
        instructions=guidance["instructions"],
        common_mistakes=guidance["common_mistakes"],
        animation_filename=exercise_guidance.get_animation_filename(name),
        calories_estimate=exercise_guidance.estimate_calories(muscle_group, sets),
    )
    ...
    for day_index, day_name in enumerate(DAY_NAMES):
        if day_index in training_indexes:
            ...
            exercises = _build_exercise_list(
                ...
                fitness_level_value=fitness_level.value,
            )
            days.append(
                WorkoutDay(
                    day_name=day_name,
                ...
                WorkoutDay(
                    ...
                    exercises=[
                        Exercise(
                            ...
                            instructions=[
                                "Go for an easy walk or do some light full-body stretching.",
                                "Keep the effort light - this should feel refreshing, not tiring.",
                            ],
                            common_mistakes=[
                                "Turning active recovery into another hard workout.",
                            ],
                            animation_filename=exercise_guidance.get_animation_filename(
                                "Light Walk or Stretching"
                            ),
                            calories_estimate=exercise_guidance.estimate_calories("Full Body", 1),
                        )
                    ],
